...
export default function KeyGenerator(props) {
    ...
    useEffect(()=>{
        axios.get('/api/users/getkeystatus',{withCredentials:true}).then(
            (data)=>{
                setOpen(!data.data.keyStatus);
            }
        )
    },[])
    /*
    ...
    return (
        <React.Fragment>
                <Dialog open={open} fullWidth>
                    <DialogTitle>Enter a Passphrase</DialogTitle>
                    <DialogContent>
                        <Typography variant="subtitle2" color="grey">Please Remember this password, If you forgot this password, you must generate new keys</Typography>
                        <TextField fullWidth type="password" onChange={handleChangePassPhrase} variant="outlined"></TextField>
                    </DialogContent>
                    <DialogActions>
                        <CustomButton text="Generate Keys" onClick={generateKeyPair} ></CustomButton>
                    </DialogActions>
                </Dialog>
        </React.Fragment>
    );
